...
def getAnnotations(chart, methods = ['GET', 'POST']):
...
@app.route("/annotations/post/<chart>/<crops>")
def postAnnotations(chart, crops, methods = ['POST']):
    all_crops = crops.lower().split(',')
    queries = [{'chart' : chart, 'commodity' : c} for c in all_crops]
    print queries
    comment = request.data.comment
    postable_queries = [query.append({'comment' : comment}) for query in queries]
